...
const normalizeCoreState = (value: Partial<AppState>): AppState => {
  ...
  const deduplicatedTransactions = (() => {
    ...
    for (const tx of nonAutoGenerated) {
      // Fingerprint ainda mais agressivo: remove espaços extras e usa normalizeText
      const cleanDesc = normalizeText(tx.description || '').replace(/\s+/g, ' ');
      const fingerprint = `${cleanDesc}_${roundCurrency(tx.amount)}_${tx.date}_${tx.type}`;
      
      const existing = seen.get(fingerprint);
      if (!existing) {
        seen.set(fingerprint, tx);
      } else {
        // Se houver conflito, prefere o que tem mais metadados ou o ID mais estável
