...
const applyRecurringProjection = (state: AppState): AppState => {
  ...
  // GLOBAL Index to avoid any duplicate collision across ALL sources
  const globalDateIndex = new Set<string>();
  // Mark all existing dates (including manual entries) as "taken"
  normalizedTransactions.forEach((tx) => {
    globalDateIndex.add(`${tx.date}_${normalizeText(tx.description || '')}_${roundCurrency(tx.amount)}`);
  });
  ...
  for (const [sourceId, list] of grouped.entries()) {
    ...
    const skips = state.skippedOccurrences?.[sourceId] || [];
    ...
    while (safety < 500) {
      ...
      // Check if this specific occurrence was manually skipped/deleted
      if (skips.includes(nextDate)) {
        // We still need to advance the cursor to project the NEXT one
        cursor = { ...cursor, date: nextDate };
        continue;
      }

      // Check global index for collision (description + amount + date)
      const collisionKey = `${nextDate}_${normalizeText(cursor.description || '')}_${roundCurrency(cursor.amount)}`;
      if (globalDateIndex.has(collisionKey)) {
        // If it collides, we don't generate, but we still advance cursor to attempt next month
        cursor = { ...cursor, date: nextDate };
        continue;
      }

      const createdAt = new Date().toISOString();
      const stableId = `auto_${sourceId}_${nextDate}`;
      ...
      globalDateIndex.add(collisionKey);
...
const normalizeCoreState = (value: Partial<AppState>): AppState => {
  ...
  const deduplicatedTransactions = (() => {
    ...
    for (const tx of nonAutoGenerated) {
      // Use a more robust fingerprint: description + amount + date + type
      // We ignore categoryId here because it might be inconsistent in duplicates
      const fingerprint = `${normalizeText(tx.description || '')}_${roundCurrency(tx.amount)}_${tx.date}_${tx.type}`;
      
      const existing = seen.get(fingerprint);
      if (!existing) {
        seen.set(fingerprint, tx);
      } else {
        // Keep the one that looks "more complete" or is older
        const existingTime = new Date(existing.createdAt || 0).getTime();
        const currentTime = new Date(tx.createdAt || 0).getTime();
        
        // If the new one is older or has better metadata (e.g. installmentGroupId), prefer it
        const currentScore = (tx.createdAt ? 1 : 0) + (tx.installmentGroupId ? 2 : 0);
        const existingScore = (existing.createdAt ? 1 : 0) + (existing.installmentGroupId ? 2 : 0);

        if (currentTime < existingTime || (currentScore > existingScore && currentTime === existingTime)) {
          seen.set(fingerprint, tx);
        }
      }
    }
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const deleteTransaction = (id: string) => {
    editState((prev) => {
      // If it's a projected transaction, we add it to skippedOccurrences
      const target = prev.transactions.find((t) => t.id === id);
      const nextSkipped = { ...prev.skippedOccurrences };

      if (target?.autoGenerated && target.recurrenceSourceId) {
        const sourceId = target.recurrenceSourceId;
        const currentSkips = nextSkipped[sourceId] || [];
        if (!currentSkips.includes(target.date)) {
          nextSkipped[sourceId] = [...currentSkips, target.date];
        }
      }
